...
# Key Pair                                #ssh-keygen -t rsa -b 4096 -f ~/.ssh/id_rsa #ls ~/.ssh/
...
#   triggers = {
#   script_hash = filemd5("script.sh") # Rerun only if script changes script_hash = filemd5("${path.module}/script.sh")
# }
}
